...
run_doctor() {
    ...
    check_symlink "~/.tool-versions" "${HOME}/.tool-versions" "${DOTFILE_PATH}/asdf/tool-versions"

    echo ""
    echo -e "  ${BOLD}Shell config${RESET}"
    check_dir    "Oh My Zsh"               "${HOME}/.oh-my-zsh"
    check_symlink "~/.zsh_preload.zsh"     "${HOME}/.zsh_preload.zsh"  "${DOTFILE_PATH}/zsh/zsh_preload.zsh"
    check_symlink "~/.zsh_postload.zsh"    "${HOME}/.zsh_postload.zsh" "${DOTFILE_PATH}/zsh/zsh_postload.zsh"
...
installZsh() {
    ...
    ln -s "${DOTFILE_PATH}/zsh/zsh_preload.zsh" "${HOME}/.zsh_preload.zsh"
    [ -L "${HOME}/.zsh_postload.zsh" ] && rm "${HOME}/.zsh_postload.zsh"
    ln -s "${DOTFILE_PATH}/zsh/zsh_postload.zsh" "${HOME}/.zsh_postload.zsh"
...
# ============================================================' \
    ...
    if [ ! -f "${HOME}/.zsh_custom.zsh" ]; then
        cp "${DOTFILE_PATH}/zsh/zsh_custom.zsh" "${HOME}/.zsh_custom.zsh"
...
installAsdf() {
    ...
    ln -s "${DOTFILE_PATH}/asdf/tool-versions" "${HOME}/.tool-versions"
    ...
    # Update asdf/tool-versions with the actually installed versions
    # Uses `asdf list <plugin> | sort -V | tail -1` to get the highest installed version reliably.
    log_info "Updating asdf/tool-versions with installed versions..."
    _update_tool_versions() {
        local plugin="$1"
        local version
        version=$(zsh -c "asdf list ${plugin} 2>/dev/null | tr -d ' *' | sort -V | tail -1")
        if [[ -n "$version" ]]; then
            if grep -q "^${plugin} " "${DOTFILE_PATH}/asdf/tool-versions"; then
                sed -i.bak "s|^${plugin} .*|${plugin} ${version}|" "${DOTFILE_PATH}/asdf/tool-versions"
                rm -f "${DOTFILE_PATH}/asdf/tool-versions.bak"
            else
                echo "${plugin} ${version}" >> "${DOTFILE_PATH}/asdf/tool-versions"
            fi
...
main() {
    ...
    # Build defaults: Core items (0-3) are checked by default, but unchecked if already installed.
    # Software/Preset items (4-11) are never checked by default.
